...
describe("Testing HelloStruct Contract", function () {
  ...
  if (!contractId) {
    throw Error(
      "HELLOSTRUCT_CONTRACT_ID: NOT FOUND IN ENV, deploy with 'make deploy-test CONTRACT=\"ContractName\"' to generate in ENV"
    );
  }
  ...
  it("Contract will revert contract state value", async () => {
    const response = await hashgraph.contract.call({
      contractId: contractId,
      method: "update",
      params: new ContractFunctionParameters().addString("hello world"),
    });
