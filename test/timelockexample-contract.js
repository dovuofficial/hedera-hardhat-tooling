...
const {
  ...
  Config,
  Hashgraph,
  SDK: { ReceiptStatusError, ContractFunctionParameters, AccountId },
...
describe("Testing a contract", function () {
  ...
  if (!contractId) {
    throw Error(
      "TIMELOCKEXAMPLE_CONTRACT_ID: NOT FOUND IN ENV, deploy with 'make deploy-test CONTRACT=\"ContractName\"' to generate in ENV"
    );
  }
  ...
  it("Add days to time lock (fails)", async () => {
    try {
      const response = await hashgraph.contract.call({
        contractId: contractId,
        method: "addDays",
        params: new ContractFunctionParameters().addUint256(1),
      });
  ...
  it("Update timelock to accept day modification", async () => {
    const response = await hashgraph.contract.call({
      ...
      params: new ContractFunctionParameters().addBool(false),
    });
  ...
  it("Revert state for ensuring contract is timelocked", async () => {
    const response = await hashgraph.contract.call({
      contractId: contractId,
      method: "updateTimeLock",
      params: new ContractFunctionParameters().addBool(true),
    });
